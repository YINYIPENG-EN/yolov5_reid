...
class ComputeLoss:
    ...
    # yolo的loss分3部分，cls, obj, loc(box)
    def __call__(self, p, targets):  # predictions, targets
        lcls = torch.zeros(1, device=self.device)  # class loss 分类回归
        lbox = torch.zeros(1, device=self.device)  # box loss 位置回归
        lobj = torch.zeros(1, device=self.device)  # object loss  置信度回归
